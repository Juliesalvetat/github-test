...
paht2data <- "C:\\Users\\jsalveta\\Desktop\\coriger\\Planilhas ajeitadas\\final\\01-12-2020\\FAROFA_underwater_video_species_observations.csv"
...
data_video$Time_UTC <- format(as.POSIXct(new_date_video_formatR), format = "%H:%M:%OS",tz='UTC') 
...
data_video_all$Date = as.character(data_video_all$Date)
x <- paste(data_video_all$Date, data_video_all$Time_UTC)
date_video_formatR <- strptime(x, "%d/%m/%Y %HH %MM %OS",tz='UTC')
data_video_all$Time_UTC <- format(as.POSIXct(date_video_formatR), format = "%H:%M:%OS",tz='UTC') 
data_video_all$Time_UTC[which(data_video_all$VideoType=="TowedVideos")] = data_video$Time_UTC;


paht2save <- "C:\\Users\\jsalveta\\Desktop\\coriger\\Planilhas ajeitadas\\final\\03-12-2020\\FAROFA_underwater_video_species_observations.csv"
